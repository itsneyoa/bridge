...
const InteractionCreate: Event<'interactionCreate'> = {
  ...
  async execute(bridge, interaction) {
    if (interaction.isAutocomplete()) {
      ...
      const choices = bridge.minecraft.guildMembers.fuse.search(focusedValue, { limit: 25 })

      await interaction.respond(
        choices.length > 0
          ? choices.map(({ item }) => ({ name: item, value: item }))
          : [...bridge.minecraft.guildMembers.get()].slice(0, 25).map(item => ({ name: item, value: item }))
      )
    }
