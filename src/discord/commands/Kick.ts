		...
		return bridge.minecraft.execute(
			{
				...
				regex: [
					{
						exp: RegExp(
							`^(?:\\[.+?\\] )?(${user}) was kicked from the guild by (?:\\[.+?\\] )?(${bridge.minecraft.username})!$`,
							"i",
						),
						exec: ([, username]) =>
							interaction.editReply({
								embeds: [
									SimpleEmbed(
										"success",
										`${inlineCode(username)} was kicked from the guild`,
									),
								],
							}),
					},
					{
						exp: /^Invalid usage! '\/guild kick <player> <reason>'$/,
						exec: () =>
							interaction.editReply({
								embeds: [SimpleEmbed("failure", "Missing reason")],
							}),
					},
