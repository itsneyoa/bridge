...
export const Mute: DiscordCommand = {
	...
	options: [
		...
		},
		{
			name: "time",
			description:
				"The time to mute for. Use m for minutes, h for hours, d for days (eg 30m)",
			type: ApplicationCommandOptionType.String,
			minLength: 1,
			maxLength: 3,
			required: true,
	...
	async execute(interaction, bridge, log) {
		...
		if (!time)
			return interaction.editReply({
				embeds: [SimpleEmbed("failure", "Time argument not found")],
			});
		...
		return bridge.minecraft.execute(
			{
				...
				regex: [
					{
						exp: RegExp(
							`^(?:\\[.+?\\] )?(?:${bridge.minecraft.username}) has muted (?:\\[.+?\\] )?(${user}) for (\\d+\\w)$`,
							"i",
						),
						exec: ([, username, time]) =>
							interaction.editReply({
								embeds: [
									SimpleEmbed(
										"success",
										`${inlineCode(username)} has been muted for ${inlineCode(time)}`,
									),
								],
							}),
					},
					{
						exp: RegExp(
							`^(?:\\[.+?\\] )?(?:${bridge.minecraft.username}) has muted the guild chat for (\\d+\\w)$`,
						),
						exec: ([, time]) =>
							interaction.editReply({
								embeds: [
									SimpleEmbed(
										"success",
										`Guild chat has been muted for ${inlineCode(time)}`,
									),
								],
							}),
					},
						...
						exec: () =>
							interaction.editReply({
								embeds: [
									SimpleEmbed(
										"failure",
										`${inlineCode(user)} is already muted`,
									),
								],
							}),
					},
					...
					},
					{
						...
						exec: () =>
							interaction.editReply({
								embeds: [SimpleEmbed("failure", "Mute length too long")],
							}),
					},
					{
						...
						exec: () =>
							interaction.editReply({
								embeds: [SimpleEmbed("failure", "Mute length too short")],
							}),
					},
					{
						...
						exec: () =>
							interaction.editReply({
								embeds: [SimpleEmbed("failure", "Invalid mute length given")],
							}),
					},
