import type { APIEmbed, EmbedField } from "discord.js";
...
export default function createLogger(bridge: Bridge) {
	return class LogGroup {
		...
		public static create(
			...
			return new LogGroup({ type, message, fields });
		}
		...
		public send() {
			...
			return LogGroup.discord.sendLog(
				this.logs.map((log) => LogGroup.buildEmbed(log)),
			);
		}
		...
		public static sendSingleLog(
			type: LogType,
			message: string,
			...fields: EmbedField[]
		) {
			console.log(LogGroup.stringify({ type, message, fields }));
			LogGroup.discord.sendLog([
				LogGroup.buildEmbed({ type, message, fields }),
			]);
		}
