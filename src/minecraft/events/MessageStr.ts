		...
		try {
			...
			if (json.server === "limbo") {
				...
				log.add(
					"info",
					[
						"Lobby detected, sending to limbo",
						...Object.entries(json).map(
							([key, value]) => `${key}: ${inlineCode(String(value))}`,
						),
					].join("\n"),
				);
			}
...
> = [
	...
	[
		...
		([, user], bridge, log) => {
			...
			return bridge.discord.sendEmbed(
				FullEmbed("success", {
					description,
					author: { name: "Member Joined!", icon_url: headUrl(user) },
				}),
				"both",
			);
		},
	...
	[
		...
		([, user], bridge, log) => {
			...
			return bridge.discord.sendEmbed(
				FullEmbed("failure", {
					description,
					author: { name: "Member Left!", icon_url: headUrl(user) },
				}),
				"both",
			);
		},
			...
			return Promise.all(
				(["guild", "officer"] as Chat[]).map((chat) =>
					bridge.discord.sendEmbed(
						FullEmbed("failure", {
							description:
								chat === "officer" ? fullDescription : baseDescription,
